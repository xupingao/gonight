...
type node struct {
	path      string        // 节点对应的路径
	indices   string        // 子节点首字母列表
	children  []*node       // 子节点列表
	handlers  HandlersChain // 与当前节点匹配的处理链
	priority  uint32        // 优先级
	nType     nodeType      // 节点类型
	maxParams uint8         // 完整路径上的最大参数数量
	wildChild bool          // 是否为通配符几点
	fullPath  string        // 该节点的完整路径
}
...
func (n *node) insertChild(numParams uint8, path string, fullPath string, handlers HandlersChain) {
	for numParams > 0 {
		...
		//  节点在被设置为通配符节点时如果存在子节点，则子节点路径将不可达
		if len(n.children) > 0 {
